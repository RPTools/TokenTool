		...
		private JPanel createContentPane() {
			...
			panel.addMouseMotionListener(new MouseMotionListener() {
				...
				public void mouseMoved(MouseEvent e) {
					...
					for (int row = 0; row < resizeBounds.length; row++) {
						for (int col = 0; col < resizeBounds[row].length; col++) {
							...
							if (rect.contains(e.getPoint())) {
								if (row == 0 && col == 0) {setCursor(Cursor.getPredefinedCursor(Cursor.NW_RESIZE_CURSOR));}
								if (row == 0 && col == 1) {setCursor(Cursor.getPredefinedCursor(Cursor.N_RESIZE_CURSOR));}
								if (row == 0 && col == 2) {setCursor(Cursor.getPredefinedCursor(Cursor.NE_RESIZE_CURSOR));}
								if (row == 1 && col == 0) {setCursor(Cursor.getPredefinedCursor(Cursor.W_RESIZE_CURSOR));}
								if (row == 1 && col == 2) {setCursor(Cursor.getPredefinedCursor(Cursor.E_RESIZE_CURSOR));}
								if (row == 2 && col == 0) {setCursor(Cursor.getPredefinedCursor(Cursor.SW_RESIZE_CURSOR));}
								if (row == 2 && col == 1) {setCursor(Cursor.getPredefinedCursor(Cursor.S_RESIZE_CURSOR));}
								if (row == 2 && col == 2) {setCursor(Cursor.getPredefinedCursor(Cursor.SE_RESIZE_CURSOR));}
								return;
							}
						}
					}
				...
				public void mouseDragged(MouseEvent e) {
					if (isDragging) {
						if (dragRow == 0 && dragCol == 0) {bounds.width += bounds.x - e.getX(); bounds.height += bounds.y - e.getY(); bounds.x = e.getX(); bounds.y = e.getY();}
						if (dragRow == 0 && dragCol == 1) {bounds.height += bounds.y - e.getY(); bounds.y = e.getY();}
						if (dragRow == 0 && dragCol == 2) {bounds.width = e.getX() - bounds.x; bounds.height += bounds.y - e.getY(); bounds.y = e.getY();}
						if (dragRow == 1 && dragCol == 0) {bounds.width += bounds.x - e.getX(); bounds.x = e.getX();}
						if (dragRow == 1 && dragCol == 2) {bounds.width = e.getX() - bounds.x;}
						if (dragRow == 2 && dragCol == 0) {bounds.width += bounds.x - e.getX(); bounds.height = e.getY() - bounds.y; bounds.x = e.getX(); }
						if (dragRow == 2 && dragCol == 1) {bounds.height = e.getY() - bounds.y;}
						if (dragRow == 2 && dragCol == 2) {bounds.height = e.getY() - bounds.y; bounds.width = e.getX() - bounds.x;}
						if (dragRow == -1 && dragCol == -1) {bounds.x = e.getX() - dragOffsetX; bounds.y = e.getY() - dragOffsetY;}
