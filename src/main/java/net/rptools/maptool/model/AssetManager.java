...
public class AssetManager {
		...
		if (asset == null && assetHasLocalReference(id)) {
			...
			if (imageFile != null) {
				...
				try {
					...
					//System.out.println("Jamz: here testing image data");
	...
	public static Asset createAsset(File file) throws IOException {
		...
		// Jamz: Need to intercept and account for non image assets
		/*System.out.println("*Jamz: file.getName() = " + file.getName());
		if (file.getName().toLowerCase().endsWith(".mp3"))
		{
			...
			{
				 mp3 = new MP3(file);
				 AttachedPicture pic = mp3.getPicture(PictureType.FRONT_COVER);
				 if (pic == null)
					 System.out.println("Jamz: Pic is null");
			...
			catch (IOException e)
			{
				...
				System.out.println("Jamz: Oh Oh");
				return new Asset(file.getName(), FileUtils.readFileToByteArray(file));	
			}

			return new Asset(file.getName(), image);
		} else {
			return new Asset(file.getName(), FileUtils.readFileToByteArray(file));	
		}*/
		return new Asset(file.getName(), FileUtils.readFileToByteArray(file));
	...
	private static void putInPersistentCache(final Asset asset) {
		...
		if (!assetInfoIsInPersistentCache(asset)) {
			...
			try {
				...
				//System.out.println("Jamz: assetName" + getAssetCacheFile(asset).getName());
